...
OUT_DIR_DEFAULT = root_dir / "artifacts" / "reports" / "secondary_layer_shadow_replay"

DEVICE = live_replay_core.DEVICE
MetaModel = live_replay_core.MetaModel
CONFIG = dict(live_replay_core.CONFIG)

# Secondary layer: solo cuando champion no entra
SECONDARY = {
...
def parse_args():
    p = argparse.ArgumentParser(description="Secondary layer shadow replay.")
...
def main():
    ...
    while i < len(df_bt) - 1:
        ...
        if take_long:
            decision = "LONG"
            size = live_replay_core.compute_side_size(
                "long", float(row["prob_long"]), CONFIG["long_threshold"],
                float(row["setup_score_long_regime"]), float(row["ev_long_regime"])
            )
            ...
            threshold = CONFIG["short_threshold_trend_down"] if str(row["regime_label"]) == "trend_down" else CONFIG["short_threshold_range"]
            decision = "SHORT"
            size = live_replay_core.compute_side_size(
                "short", float(row["prob_short"]), threshold,
                float(row["setup_score_short_regime"]), float(row["ev_short_regime"])
            )
            decision_source = "primary"
            decision_reason = "passed_all" if decision_reason == "no_side_passed" else decision_reason
            counters["primary_passed"] += 1
        ...
        if decision == "NONE":
            ...
            if take_sec_long:
                decision = "LONG"
                size = min(
                    SECONDARY["long_max_size"],
                    live_replay_core.compute_side_size(
                        "long", float(row["prob_long"]), SECONDARY["long_threshold"],
                        float(row["setup_score_long_regime"]), float(row["ev_long_regime"])
                    )
                )
                ...
                decision = "SHORT"
                size = min(
                    SECONDARY["short_max_size"],
                    live_replay_core.compute_side_size(
                        "short", float(row["prob_short"]), SECONDARY["short_threshold_range"],
                        float(row["setup_score_short_regime"]), float(row["ev_short_regime"])
                    )
                )
                decision_source = "secondary"
