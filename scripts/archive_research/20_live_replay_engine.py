...
OUT_DIR_DEFAULT = root_dir / "artifacts/reports/live_replay_engine"
...
def parse_args():
    parser = argparse.ArgumentParser(description="Replay histórico barra a barra del champion actual.")
    parser.add_argument("--bars", type=int, default=1000, help="Número de barras finales a procesar.")
    parser.add_argument("--start", type=str, default="", help="Fecha inicio YYYY-MM-DD opcional.")
    parser.add_argument("--end", type=str, default="", help="Fecha fin YYYY-MM-DD opcional.")
...
def format_heartbeat(row: pd.Series, long_d: Dict[str, Any], short_d: Dict[str, Any],
    ...
    return "\n".join([
        ...
        f"LONG: candidate={long_d['candidate']} selected_regime={long_d['selected_regime']} prob={long_d['prob']:.4f} ev={long_d['ev']:.5f} score={long_d['score']:.4f} thr_ok={long_d['threshold_ok']} margin_ok={long_d['margin_ok']} ev_ok={long_d['ev_ok']} score_ok={long_d['score_ok']} reason={long_d['reason']}",
        f"SHORT: candidate={short_d['candidate']} selected_regime={short_d['selected_regime']} prob={short_d['prob']:.4f} ev={short_d['ev']:.5f} score={short_d['score']:.4f} thr_ok={short_d['threshold_ok']} margin_ok={short_d['margin_ok']} ev_ok={short_d['ev_ok']} score_ok={short_d['score_ok']} reason={short_d['reason']}",
...
def main():
    ...
    logger.info("=" * 100)
    logger.info("LIVE REPLAY ENGINE - CHAMPION ACTUAL")
    logger.info("=" * 100)
    ...
    if args.start or args.end:
        ...
        df_replay = df.iloc[-bars:].copy().reset_index(drop=True)
    ...
    X_long = apply_robust_scaler(df_replay, long_feature_cols, long_scaler)
    X_short = apply_robust_scaler(df_replay, short_feature_cols, short_scaler)
    ...
    logger.info("=" * 100)
    logger.info("LIVE REPLAY SUMMARY")
    logger.info("=" * 100)
