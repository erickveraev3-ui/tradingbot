...
REPLAY_SCRIPT = root_dir / "scripts" / "20_live_replay_engine.py"
OUT_DIR = root_dir / "artifacts" / "reports" / "rolling_replay_scanner"


def parse_args():
    p = argparse.ArgumentParser(description="Rolling replay scanner del champion.")
...
def main():
    ...
    out_df.to_csv(OUT_DIR / "rolling_summary.csv", index=False)
    ...
    if len(out_df):
        print("\nAGGREGATES")
        print(out_df[["total_return", "max_drawdown", "sharpe_ratio", "n_trades"]].describe().to_string())
