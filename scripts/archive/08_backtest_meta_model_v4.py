...
from src.strategy.setup_ranking import compute_setup_scores, select_top_setups
...
CONFIG = {
    ...
    # no queremos hiperfiltro por threshold duro
    "threshold_long": 0.55,
    "threshold_short": 0.55,

    # ventaja relativa mínima
    "prob_margin": 0.02,

    # selección por ranking
    "top_percent_long": 0.20,   # top 20% de candidatos long
    "top_percent_short": 0.20,  # top 20% de candidatos short
...
def main():
    logger.info("=" * 80)
    logger.info("BACKTEST META MODEL V4")
    logger.info("=" * 80)
    ...
    df_bt = select_top_setups(df_bt, side="long", top_percent=CONFIG["top_percent_long"], candidate_col="candidate_long")
    df_bt = select_top_setups(df_bt, side="short", top_percent=CONFIG["top_percent_short"], candidate_col="candidate_short")
    ...
    while i < len(df_bt) - 1:
        ...
        if take_long and take_short:
            # elegimos el lado con mejor edge relativo
            if (score_long - score_short) >= 0:
                take_short = False
            else:
                take_long = False
    ...
    }).to_csv(REPORT_DIR / "backtest_meta_model_v4_equity.csv", index=False)

    trades_df.to_csv(REPORT_DIR / "backtest_meta_model_v4_trades.csv", index=False)

    with open(REPORT_DIR / "backtest_meta_model_v4_metrics.json", "w", encoding="utf-8") as f:
        json.dump(metrics, f, ensure_ascii=False, indent=2)

    logger.info("=" * 80)
    logger.info("RESULTADOS BACKTEST META MODEL V4")
    logger.info("=" * 80)
