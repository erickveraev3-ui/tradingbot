...
from src.strategy.expected_value_engine import compute_expected_values, select_ev_trades
...
CONFIG = {
    ...
    # ya no es el filtro principal, solo sanidad mínima
    "threshold_long": 0.50,
    "threshold_short": 0.50,

    # margen mínimo entre lados
    "prob_margin": 0.00,
    ...
    # para convertir ATR barriers a % esperado
...
def main():
    logger.info("=" * 80)
    logger.info("BACKTEST META MODEL V5 - EXPECTED VALUE")
    logger.info("=" * 80)
    ...
    # convertir ATR barriers esperadas a % sobre precio de entrada
    ...
    df_bt = compute_expected_values(
        df_bt,
        roundtrip_cost_pct=roundtrip_cost_pct,
    )

    df_bt = select_ev_trades(
        df_bt,
        min_ev_net=CONFIG["min_ev_net"],
        top_percent_long=CONFIG["top_percent_long"],
        top_percent_short=CONFIG["top_percent_short"],
    )
    ...
    while i < len(df_bt) - 1:
        ...
        score_long = float(row["setup_score_long"])
        score_short = float(row["setup_score_short"])
        ev_long = float(row["ev_long_net"])
        ev_short = float(row["ev_short_net"])

        take_long = (
            CONFIG["use_long"]
            and int(row["selected_long_ev"]) == 1
        ...
        take_short = (
            CONFIG["use_short"]
            and int(row["selected_short_ev"]) == 1
    ...
    metrics = {
        ...
        "avg_ev_long": float(trades_df["ev_long_net"].mean()) if len(trades_df) else 0.0,
        "avg_ev_short": float(trades_df["ev_short_net"].mean()) if len(trades_df) else 0.0,
    ...
    }).to_csv(REPORT_DIR / "backtest_meta_model_v5_equity.csv", index=False)

    trades_df.to_csv(REPORT_DIR / "backtest_meta_model_v5_trades.csv", index=False)

    with open(REPORT_DIR / "backtest_meta_model_v5_metrics.json", "w", encoding="utf-8") as f:
        json.dump(metrics, f, ensure_ascii=False, indent=2)

    logger.info("=" * 80)
    logger.info("RESULTADOS BACKTEST META MODEL V5")
    logger.info("=" * 80)
