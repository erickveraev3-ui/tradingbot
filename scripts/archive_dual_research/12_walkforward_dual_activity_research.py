...
OUT_DIR = root_dir / "artifacts" / "reports" / "walkforward_dual_activity_research"
...
CONFIG["modes"] = [
    "short_only_baseline",
    "dual_long_trendup_loose",
    "dual_long_trendup_hard",
]

# ---------------------------------------------------------------------
# Silenciar FutureWarning de pandas en attribution
# ---------------------------------------------------------------------
...
# Short queda intacto como benchmark ganador.
# ---------------------------------------------------------------------
def evaluate_long_pass(row, mode):
    ...
    # ------------------------------------------------------------
    # Variante loose: long solo en trend_up, pero relativamente abierto
    # ------------------------------------------------------------
    if mode == "dual_long_trendup_loose":
        if regime != "trend_up":
    ...
    # ------------------------------------------------------------
    # Variante hard: trend_up solamente, pero long mucho más exigente
    # Esta es la pata que intentamos rescatar sin dañar el short.
    # ------------------------------------------------------------
    if mode == "dual_long_trendup_hard":
        if regime != "trend_up":
            return False, {"reason": "regime_block", "regime": regime}
...
# ---------------------------------------------------------------------
# Penalizar el long en conflictos duales.
# Idea: si ambos pasan, el long solo gana si su señal es claramente mejor.
# ---------------------------------------------------------------------
...
def dual_conflict_rank(direction: str, row: pd.Series) -> float:
    ...
    mode = None
    # el base script no pasa mode aquí, así que usamos solo penalización estructural
    # al long cuando el short también es fuerte.
    if direction == "long":
        p_long = float(row["prob_long"])
        p_short = float(row["prob_short"])
        ev_long = float(row["ev_long_regime"])
        ...
        # si short es competitivo, restamos prioridad al long
        penalty = 0.0
        if p_short >= 0.53:
            penalty += 0.0015
        if ev_short >= 0.0045:
...
# ---------------------------------------------------------------------
# Diagnóstico de actividad operativa
# Esto es crítico por la lección de los bots antiguos:
# no queremos un sistema que "parezca bueno" pero llegue a live y no opere.
# ---------------------------------------------------------------------
...
for mode in CONFIG["modes"]:
    ...
    # criterio de "actividad razonable" para no repetir live muerto
    # No es criterio final de trading, es un criterio de seguridad operativa.
...
print("\n" + "=" * 120)
print("LIVE SAFETY REMINDER")
print("=" * 120)
print(
    "Antes de promover cualquier modo a paper/live, exigir:\n"
    "1) Paridad research/live sobre el mismo tramo histórico.\n"
    "2) Live leyendo velas cerradas directamente del exchange, no CSV congelado.\n"
    "3) Heartbeat por vela cerrada con reason codes cuando no hay trade.\n"
    "4) Activity gate superado o, si no se supera, justificar que la baja frecuencia es intencional.\n"
)
