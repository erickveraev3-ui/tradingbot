...
OUT_DIR = root_dir / "artifacts" / "reports" / "walkforward_dual_short_refine"
...
CONFIG["modes"] = [
    ...
    "dual_short_refine_range",
    "dual_short_refine_both",
]
...
# ---------------------------------------------------------------------
# LONG fijo: mantener exactamente el embudo elite ganador
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# SHORT refinado por modo
# El baseline short_only queda tal como funciona.
# En dual refinamos el embudo short sin tocar el long elite.
# ---------------------------------------------------------------------
def evaluate_short_pass(row, mode):
    ...
    if mode == "short_only_baseline":
    ...
    elif mode == "dual_long_trendup_elite_ref":
        if regime not in ("trend_down", "range"):
            return False, {"reason": "regime_block", "regime": regime}
    ...
    elif mode == "dual_short_refine_range":
        if regime not in ("trend_down", "range"):
            return False, {"reason": "regime_block", "regime": regime}
        ...
        if regime == "trend_down":
            ...
            # range más estricto: queremos cortar short marginal
            params = {"threshold": 0.55, "prob_margin": 0.05, "min_ev": 0.0058, "min_setup": 1.80}

    elif mode == "dual_short_refine_both":
        if regime not in ("trend_down", "range"):
            return False, {"reason": "regime_block", "regime": regime}

        if regime == "trend_down":
            # un poco más exigente para limpiar algo de basura sin matar el core
            params = {"threshold": 0.53, "prob_margin": 0.035, "min_ev": 0.0042, "min_setup": 1.45}
        else:
            params = {"threshold": 0.55, "prob_margin": 0.05, "min_ev": 0.0058, "min_setup": 1.80}
...
# ---------------------------------------------------------------------
# Conflicto dual
# Mantener preferencia estructural por short salvo que el long sea muy claro.
# ---------------------------------------------------------------------
def dual_conflict_rank(direction: str, row: pd.Series) -> float:
    if direction == "long":
        ...
        # long pierde desempates si short es competitivo
        if p_short >= 0.53:
            rank -= 0.0015
...
print("\n" + "=" * 120)
print("DUAL SHORT REFINE GOAL")
print("=" * 120)
print(
    "Buscamos mejorar dual_long_trendup_elite_ref refinando solo el short dentro de dual,\n"
    "sin tocar el long elite y comparando siempre contra short_only_baseline congelado.\n"
)
